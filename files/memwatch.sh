...
#     (logs/archive/vllm-fn-tp1-20260919T222930-memwatch.log): a vision request
#     held MemFree at 1.2-1.5 GiB for 5 s while MemAvailable stayed at ~7 GiB
#     (5+ GiB of reclaimable cache), zero NV_ERR_NO_MEMORY, serving clean.
#     Sub-floor-but-reclaimable is a load transient, not the exhaustion the
#     trigger exists for. The trigger now also requires EVIDENCE that the host
#     is actually exhausting, counted only while at least one holds:
